...
python ${isofit_base_path}/utils/apply_oe.py small_chunk/${file_base}_rdn_7000-7010 small_chunk/${file_base}_loc_7000-7010 small_chunk/${file_base}_obs_7000-7010 small_chunk_test_modtran ang --presolve=1 --empirical_line=0 --n_cores ${n_cores} --surface_path configs/basic_surface.mat --copy_input_files 0

# Medium test (1000x598 pixels, empirical line) - this should take ~45 minutes with n_cores = 4
python ${isofit_base_path}/utils/apply_oe.py medium_chunk/${file_base}_rdn_7k-8k medium_chunk/${file_base}_loc_7k-8k medium_chunk/${file_base}_obs_7k-8k medium_chunk_test_modtran ang --presolve=1 --empirical_line=1 --n_cores ${n_cores} --surface_path configs/basic_surface.mat --segmentation_size 400
